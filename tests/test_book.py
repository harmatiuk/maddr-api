...
def test_read_all_books_with_year_filter(client, token, book):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = client.get(
        f"/book/?publish_year={book.publish_year}&skip=0&limit=20",
        headers=headers,
    )
    expected_response = [
        dict(
...
def test_read_all_books_with_title_filter(client, token, book):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = client.get(
        f"/book/?title={book.title}&skip=0&limit=20", headers=headers
    )
    expected_response = [
        dict(
            id=book.id,
...
def test_read_all_books_title_not_found(client, token):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = client.get(
        "/book/?title=NonExistentBook&skip=0&limit=20", headers=headers
    )
...
def test_read_all_books_year_not_found(client, token):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = client.get(
        "/book/?publish_year=1999&skip=0&limit=20", headers=headers
    )
